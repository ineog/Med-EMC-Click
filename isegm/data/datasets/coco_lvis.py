...
class CocoLvisDataset(ISDataset):
    def __init__(self, dataset_path, split='train',img_split='train2017',stuff_prob=0.0,
        ...
        self._images_path = self.img_split_path
    ...
    def get_sample(self, index) -> DSample:
        ...
        layerss = [cv2.imdecode(x, cv2.IMREAD_UNCHANGED) for x in encoded_layers]
        layers = np.stack(layerss, axis=2)
        ...
        #if len(objs_mapping) == 1:
            #print("Hola")
        print("layers shape",np.shape(layerss))
            #layers = np.stack(layers, axis=2)
            #print("layers stack shape",np.shape(layers))
            #print("image shape", image.shape)
            #print("instances_info", instances_info)
        #print("Instances info2:", instances_info)
        #return DSample(image, layers, objects_ids=np.unique(layers), sample_id=index)
        print(f"instances_info: {len(instances_info)}")
