...
class ISDataset(torch.utils.data.dataset.Dataset):
        ...
        while(1):
            try:
                ...
                if self.with_refiner:
                    ...
                    if mask.sum() > self.min_object_area and mask.sum() < mask_area * 0.15:
                    ...
                    #print(f"mask sum: {mask.sum()}\nmin area: {self.min_object_area}\nmask area: {mask_area*1}")
                    if mask.sum() > self.min_object_area and mask.sum() < mask_area*1:
                        output = {
                            'trimap': trimap,
                            'images': self.to_tensor(image),
                            'points': points.astype(np.float32),
                ...
                #print(e.args)
                #print(str(e))
                print('.............', e.args, ' ', str(e), ' ', repr(e), "index:", index)
                #print("error con", index)
                traceback.print_exc()
                index = np.random.randint(len(self.dataset_samples)-1)
    ...
    def sampling_roi_full_object(self, gt_mask, min_size=32):
        ...
        ratio = np.random.randint(13,20)/10
    ...
    def augment_sample(self, sample) -> DSample:
        #print("Sample entrada:", sample, dir(sample))
        ...
        while not valid_augmentation:
            ...
            #print("Sample salida", sample, dir(sample))
